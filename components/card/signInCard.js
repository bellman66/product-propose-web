...
function SignInCard() {
    ...
    const handleInputById = (e) => {
        setSignInForm(original => ({
            ...original,
    ...
    const clickSignInBtn = async (e) => {
        await axios.post('http://localhost:8080/api/v1/account/login', signInForm)
                    .then(res => {
    ...
    return (
        <div className="bg-white shadow-lg rounded xl:w-1/3 lg:w-5/12 md:w-1/2 w-full lg:px-10 sm:px-6 sm:py-10 px-2 py-6">
            ...
            {/* <label>Output:</label>
            <pre>{JSON.stringify(signInForm, null, 2)}</pre> */}
            ...
            <div>
                ...
                <input id="email" 
                       onChange={handleInputById} 
                       aria-labelledby="email" type="email" className="bg-gray-200 border rounded text-xs font-medium leading-none placeholder-gray-800 text-gray-800 py-3 w-full pl-3 mt-2" placeholder="e.g: john@gmail.com " />
            </div>
